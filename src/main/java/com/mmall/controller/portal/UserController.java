 ...
 * @Author : Isaac
 * @Description:
 * @Date :Created in 13:13 2018/3/13
 */
@Controller
@RequestMapping("/user/")
...
//            session.setAttribute(Const.CURRENT_USER, response.getData());
    ...
    @ResponseBody
    public ServerResponse<String> logout( HttpServletRequest request,HttpServletResponse httpServletResponse) {
    ...
    public ServerResponse<User> getUserInfo(HttpSession session,HttpServletRequest request) {
        String loginTonken = CookieUtil.readLoginCookie(request);
        if (StringUtils.isEmpty(loginTonken)) {
            return ServerResponse.createByErrorMessage("用户未登录,无法获取当前用户的信息");
        }
        String  userJsonStr = RedisPoolUtil.get(loginTonken);
    ...
    public ServerResponse<User> updateInformation(HttpServletRequest request,User user){
        ...
        if(response.isSuccess()){
            ...
            RedisPoolUtil.setEx(loginTonken, JsonUtil.obj2str(response.getData()), Const.RedisCacheExtime.REDIS_SESSION_EXTIME);
        }
        return response;
